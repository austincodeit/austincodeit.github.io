...
console.log("%c#######################################################",b);console.log("%c#######%c hi! welcome to austin code's github!!! %c#######",b,n,b);console.log("%c#######################################################",b);$("#footnoteInsert").html(''+'<span class="label label-default">Last Update: '+'<span id="dateString">'+DATE_STRING+'</span>'+'</span>');});$(document).ready(function(){let openDataLink='https://data.austintexas.gov/resource/czdh-pagu.json';let openData;$.ajax({url:openDataLink,type:"GET",data:{"$limit":7000,"$$app_token":"AmHlGm0OHBl6r4hg0PLvAtJk7"}}).done(function(data){let nameArray=_.chain(data).pluck('assigneduser').uniq().value();let removeArray=['Todd Wilcox','Viola Ruiz','Marcus Elliott','Tammy Lewis','Kendrick Barnett',];let filterArray=nameArray.filter(function(name){if(removeArray.indexOf(name)<0){return name;}})
